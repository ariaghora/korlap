...
// ── Repo-level script commands ──────────────────────────────────────

fn repo_script_key(repo_id: &str) -> String {
    format!("repo:{}", repo_id)
}
...
pub fn run_repo_script(
    ...
    let key = repo_script_key(&repo_id);
    ...
    {
        let mut st = state.lock().map_err(|e| e.to_string())?;
        st.script_pids.insert(key.clone(), pid);
    }
    ...
    std::thread::spawn(move || {
        ...
        if let Ok(mut st) = state_clone.lock() {
            st.script_pids.remove(&cleanup_key);
        }
    });
...
pub fn stop_repo_script(
    ...
    let pid = {
        let st = state.lock().map_err(|e| e.to_string())?;
        st.script_pids.get(&key).copied()
    };

    let pid = pid.ok_or("No running script for this repo")?;
